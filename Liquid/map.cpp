...
void Map::drawGoal()
{
	for (int Ver = 0; Ver < MAPDATA_V_MAX; Ver++)
	{
		for (int Hor = 0; Hor < MAPDATA_H_MAX; Hor++)
		{
			...
			{
				DrawRectGraph(Map::getInstance()->rel_posX + Hor * CHIP_SIZE, MAP_DRAW_POSITION_Y + Ver * CHIP_SIZE - 24, CHIP_SIZE*6, 360, CHIP_SIZE, CHIP_SIZE + 24, handle, TRUE, FALSE);
			}
		}
	}
}
